          ...
          return (
            <li key={m.id}>
              <Link href={`/match/${m.slug}`} className="block py-2.5">
                ...
                <ResultRow name={m.homeTeam?.name ?? "Home"} crest={m.homeTeam?.crest} score={m.homeScore} win={h > a} />
                <ResultRow name={m.awayTeam?.name ?? "Away"} crest={m.awayTeam?.crest} score={m.awayScore} win={a > h} />
              </Link>
            </li>
          );
        })}
...
function ResultRow({ name, crest, score, win }: { name: string; crest?: string; score?: number; win: boolean }) {
